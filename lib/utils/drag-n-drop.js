...
var DragHandlers = (function DragHandlers() {
	...
	var start = function start(objectUID, e) {
		DragedElement = this;
		parentElement = this.parentElement;
		DragedElement.classList.add('drag');
		e.dataTransfer.effectAllowed = 'copy';
		e.dataTransfer.setData('UID', objectUID);
	...
	var setDraggable = function setDraggable(node, objectUID, callback) {
		node.setAttribute('draggable', 'true');
		node.addEventListener('dragstart', start.bind(node, objectUID));
